...
export function parseDraftReport(text: string): DraftReport {
  ...
  if (!isRecord(value) || !Array.isArray(value.drafts)) {
    throw new Error("Draft report must contain drafts");
  }
  for (const [index, candidate] of value.drafts.entries()) {
...
export async function parseWorkInput(args: string[]): Promise<WorkInput> {
  let target = ".";
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (["--ja", "-j", "--japanese"].includes(arg)) continue;
    if (arg === "--target" || arg === "--file" || arg === "-f") {
      const value = args[++i];
      ...
      throw new Error("Use either a single quoted TODO text or --file <path>");
    } else {
      positional = arg;
    }
  }

  if (todoFile && positional !== undefined) {
    throw new Error("Use either TODO text or --file, not both");
  }

  todo = todoFile ? await Bun.file(todoFile).text() : positional;
  if (!todo?.trim()) {
    throw new Error("TODO items required: provide text as an argument or via --file <path>");
  }

  return { target_path: target, todo: todo.trim() };
}
...
async function initializeLocks() {
  ...
  lockDirectory = join(common.stdout.trim(), "task-locks");
}
...
export async function runToolToFile(
  ...
  const proc = Bun.spawn(["bash", "-c", fullCmd, "task", prompt, absOutputFile, transcriptFile], {
    cwd: cwd || commandCwd,
    detached: true,
    stdin: "ignore",
    stdout: "ignore",
...
async function cmdStatus(targetRun?: string, summaryOnly = false, statusFilter?: string) {
  ...
  if (runs.length === 0) {
    console.log(`No iterations found in ${OUTPUT_DIR}. Run ./scripts/task.ts run to start.`);
    return;
  }

  if (!summaryOnly) {
    const filterInfo = statusFilter ? ` (Filter: ${statusFilter.toUpperCase()})` : "";
    console.log(`\n${BOLD}${CYAN}#${NC} ${BOLD}${WHITE}Task Orchestration Status${filterInfo}${NC}`);
  }
  ...
  for (const r of runs) {
    ...
    for (const p of proposals) {
      ...
      const issue = p.issue || "";
      const proposal = p.instructions || (p.plan || []).join("\n") || "";
    ...
    if (summaryOnly) {
      if (statusFilter && visibleTasks.length === 0) continue;
...
async function removeIterationFiles(rDir: string, iterName: string): Promise<{ branches: string[]; worktrees: number }> {
  ...
  if (state.branch_prefix) {
    ...
    if (iterNum !== 99999) {
      patterns.add(`task/iter-${iterNum}/*`);
      patterns.add(`task/iter-${iterNum}`);
      patterns.add(`todo/iter-${iterNum}/*`);
      patterns.add(`todo/iter-${iterNum}`);
    }
    patterns.add(`task/${iterName}/*`);
    patterns.add(`task/${iterName}`);
    patterns.add(`todo/${iterName}/*`);
    patterns.add(`todo/${iterName}`);
  }
...
export function formatJsonlLine(line: string, previousOutput = ""): string | null {
  ...
  try {
    ...
    if (obj.source === "MODEL" && obj.content && typeof obj.content === "string") {
      ...
      if (obj.type === "ARCHITECT_RESPONSE" || obj.type === "PLANNER_RESPONSE") {
        results.push(`${GREEN}[MSG]${NC}   ${firstLine}`);
...
async function cmdWatch(targetRun?: string, targetTask?: string) {
  ...
  logInfo("Watching TODO task log in real-time (Press Ctrl+C to stop)...");
      ...
      if (!logFile) {
        for (const dirName of allDirs.slice().reverse()) {
          ...
          if (await taskLogFile.exists()) {
            ...
            runId = dirName;
            phase = "phase3";
            worktreeDir = join(iterDir, "worktrees", targetTask);
            break;
          }
        }
      }
...
function printUsage() {
  console.log(`Usage: ./scripts/task.ts <COMMAND> [OPTIONS] [TODO_TEXT]

Commands:
  run [OPTIONS] [TODO_TEXT]       Draft, refine, implement, and review TODO tasks
  run [OPTIONS] --file PATH       Read TODO items from a file
...
Options:
  --file, -f PATH                 File containing TODO items
  --target PATH                   Scope or directory to inspect (default: .)
  --ja, -j                        Generate agent reports in Japanese; system logs stay English
  TODO_TEXT                       Raw TODO items string

Environment variables:
  PLANNER                         Cheap/fast model: draft plans from TODO items (required)
  ARCHITECT                       Strong model: refine and approve execution plans (required)
...
Example:
  PLANNER='pi -m gemini-2.5-flash -p' \\
  ARCHITECT='pi -m claude-3-7-sonnet -p' \\
  DEVELOPER='pi -m gemini-2.5-flash -p' \\
  REVIEWER='pi -m claude-3-7-sonnet -p' \\
  ./scripts/task.ts run --file tasks.md`);
}
...
export async function main() {
  ...
  if (cmd === "status" || cmd === "--status") {
    ...
    for (let i = 1; i < args.length; i++) {
      ...
      if (a.startsWith("--filter=")) {
        ...
        if (i + 1 < args.length) statusFilter = args[++i];
    ...
    if (positional.length === 1) {
      ...
      target = positional[0];
      if (!statusFilter) statusFilter = positional[1];
    }
  ...
  let input: WorkInput = { target_path: ".", todo: "" };
  ...
  if (resumeDir) {
    ...
    input = saved.input || { target_path: saved.target_path || ".", todo: "" };
  }
  ...
  try {
    ...
    let draftsCompleted = false;
    let branchPrefix = `task/${randomUUID()}`;
    ...
    if (await stateFile.exists()) {
      ...
      draftsCompleted = existingState.phase1?.status === "completed";
      input = existingState.input || input;
      branchPrefix = existingState.branch_prefix || branchPrefix;
    ...
    const draftsPath = join(runDir, "phase1_drafts.json");
    ...
    // --- Phase 1: Draft Plans (PLANNER: Cheap/Fast model) ---
    if (draftsCompleted && (await Bun.file(draftsPath).exists()) && Bun.file(draftsPath).size > 0) {
      logOk(`Phase 1: Existing draft plans found in ${draftsPath}. Skipping drafting.`);
    } else {
      logInfo(`Phase 1: Drafting execution plans with PLANNER (${PLANNER})...`);
      await setCurrentTask(basename(runDir), iteration, "phase1", `Phase 1: Drafting plans with PLANNER`, draftsPath);

      const plannerPrompt = `You are an implementation planner. Read AGENTS.md and inspect '${targetPath}'.
Target scope: ${targetPath}

The user has supplied the following TODO items to implement:
---
${input.todo}
---

Break down the TODO items into independent, concrete implementation plans.
For each item, inspect the relevant codebase, identify the target files to touch, and draft a step-by-step implementation plan with acceptance criteria.
...
${isIterJa ? "Write descriptive values in Japanese; keep JSON keys, IDs, code symbols, and paths unchanged." : ""}`;
      ...
      const exitCode = await runToolToFile(PLANNER, plannerPrompt, draftsPath, undefined, async (pid) => {
        await setCurrentTask(basename(runDir), iteration, "phase1", `Phase 1: Drafting plans with PLANNER`, draftsPath, "running", undefined, pid);
      });
      const outputText = (await Bun.file(draftsPath).exists()) ? await Bun.file(draftsPath).text() : "";
      ...
      if (isQuotaExhausted(outputText)) {
        ...
        quotaExhausted = true;
      } else if (exitCode !== 0 || !outputText.trim()) {
        logError(`PLANNER failed with exit code ${exitCode}. Check ${draftsPath}`);
        await updateRunState(runDir, { status: "failed" });
      } else {
        try {
          const draftReport = parseDraftReport(outputText);
          await Bun.write(draftsPath, JSON.stringify(draftReport, null, 2));
          logOk(`Phase 1 complete. ${draftReport.drafts.length} draft plan(s) saved to ${draftsPath}`);
        } catch (err) {
          logError(`PLANNER returned an invalid JSON report: ${err}. Check ${draftsPath}`);
          await updateRunState(runDir, { status: "failed" });
        }
      }
    }

    const stateAfterP1: IterationState = await Bun.file(statePath).json();
    if (stateAfterP1.status === "failed") {
      process.exit(1);
    }
    await updateRunState(runDir, { phase1: { status: "completed" } });

    // --- Phase 2: Plan Review & Refinement (ARCHITECT: Strong model) ---
...
${isIterJa ? "Write descriptive values in Japanese; keep JSON keys, IDs, code symbols, and paths unchanged." : ""}`;
    ...
    const stateAfterP2: IterationState = await Bun.file(statePath).json();
    if (stateAfterP2.status === "failed") {
      process.exit(1);
    }
    await updateRunState(runDir, { phase2: { status: "completed" } });
    await cmdStatus(basename(runDir), true);

    // --- Phase 3: Parallel Implementation in Git Worktrees (DEVELOPER: Cheap/Fast model) ---
...
${isIterJa ? "Write summary and reasons in Japanese; keep status values in English." : ""}`;
    ...
    // Phase 5: Clean up worktrees and temporary branches
  ...
  console.log("");
  logInfo("========================================================");
  logInfo("Task Orchestration Run Summary");
  logInfo("========================================================");
  ...
  logInfo(`To view iteration task statuses: ./scripts/task.ts status`);
